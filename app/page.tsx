...
      if (game.turn() === "w") setWhiteTime(p => { if (p <= 1) { setIsGameActive(false); setGameResult({ winner: "Black", reason: "on time" }); return 0 }; return p - 1 })
      else setBlackTime(p => { if (p <= 1) { setIsGameActive(false); setGameResult({ winner: "White", reason: "on time" }); return 0 }; return p - 1 })
    }, 1000)
...
  // Ход бота (Stockfish)
  useEffect(() => {
    if (screen === "game" && gameMode === "pve" && isGameActive && !isPaused && !game.isGameOver()) {
      if (game.turn() === "b" && !isBotThinking) {
        const fetchBotMove = async () => {
          setIsBotThinking(true);
          try {
            const res = await fetch(`https://stockfish.online/api/s/v2.php?fen=${encodeURIComponent(game.fen())}&depth=10`);
...
                
                if (game.isCheckmate()) { setGameResult({ winner: "White", reason: "checkmate" }); setIsGameActive(false); }
                else if (game.isDraw() || game.isStalemate()) { setGameResult({ winner: "Draw", reason: "draw" }); setIsGameActive(false); }
              }
            }
          } catch (error) {
...
  // Обращение к нашему новому ИИ-тренеру на Next.js API
...
          if (game.isCheckmate()) { setGameResult({ winner: game.turn() === "w" ? "Black" : "White", reason: "checkmate" }); setIsGameActive(false) }
          else if (game.isStalemate()) { setGameResult({ winner: "Draw", reason: "stalemate" }); setIsGameActive(false) }
          else if (game.isDraw()) { setGameResult({ winner: "Draw", reason: "draw" }); setIsGameActive(false) }
          
          setGame(new Chess(game.fen()))
        }
      } catch {}
...
        <div style={{ minHeight: "100vh", backgroundColor: theme.boardColors.pageBg, display: "flex", flexDirection: "column", alignItems: "center", justifyItems: "center", padding: "2rem", color: text, transition: "background-color 0.4s ease", justifyContent: "center" }}>
...
      <div className="chess-root" style={{ backgroundColor: theme.boardColors.pageBg }}>
...
                  ● Black {isBotThinking && "(AI is thinking...)"}
                </span>
                <span className="chess-mono" style={{ fontSize: "1rem" }}>{formatTime(blackTime)}</span>
              </div>

              <div style={{ width: "100%", aspectRatio: "1", display: "grid", gridTemplateColumns: "repeat(8,1fr)", gridTemplateRows: "repeat(8,1fr)", border: `2px solid ${panelBorder}` }}>
...
                  {game.isCheckmate() ? `Checkmate — ${game.turn() === "w" ? "Black" : "White"} wins.` : game.isDraw() ? "A drawn position." : game.isCheck() ? `${game.turn() === "w" ? "White" : "Black"} in check.` : `${game.turn() === "w" ? "White" : "Black"} to move`}
                </p>
              </div>
...
        {settingsOpen && (
          <div className="settings-overlay" onClick={() => { setIsPaused(false); setSettingsOpen(false) }} style={{ zIndex: 1000, position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)' }}>
            <div style={{ background: theme.boardColors.pageBg, padding: '2rem', borderRadius: '8px', border: `1px solid ${panelBorder}`, margin: '10% auto', width: '300px', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
               <button className="menu-btn" style={{ marginBottom: "1rem" }} onClick={() => { setIsPaused(false); setSettingsOpen(false) }}>Resume Game</button>
