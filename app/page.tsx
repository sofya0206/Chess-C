...
function generateComment(move: Move, game: Chess, lastComment: string, usedRef: React.MutableRefObject<Set<string>>): string {
  ...
  if (candidates.length === 0) { usedRef.current.clear(); candidates = primaryPool.filter((c) => c !== lastComment) }
        ...
        {gameResult && (
          <div className="result-overlay">
            <div className="result-modal">
              <p className="chess-mono" style={{ fontSize: "0.65rem", letterSpacing: "0.25em", textTransform: "uppercase", color: `${text}60`, margin: "0 0 1.2rem 0" }}>
                {gameResult.reason}
              </p>
              <p className="chess-serif-display" style={{ fontSize: gameResult.winner === "Draw" ? "2.4rem" : "2.8rem", fontStyle: "italic", color: text, margin: 0, lineHeight: 1.1 }}>
                {gameResult.winner === "Draw" ? "Draw" : gameResult.winner}
              </p>
              {gameResult.winner !== "Draw" && (
                <p className="chess-serif-display" style={{ fontSize: "1.1rem", fontStyle: "italic", color: `${text}70`, margin: "0.5rem 0 0 0" }}>
                  wins the game
                </p>
              )}
